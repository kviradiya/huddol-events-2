...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	...
	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
	    ...
	    {
		    $link = '<a rel="nofollow" class="comment-reply-login" href="' . esc_url( site_url() . "/login/" ) . '">' . $args['login_text'] . '</a>';
		}
		else
		{
...
function comment_form( $args = array(), $post_id = null ) {
	...
	if(ICL_LANGUAGE_CODE == 'en')
	{
	    $must = '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), site_url() . "/login/?redirect=" . get_permalink($post_id) ) . '</p>';
	}
	else
	{
