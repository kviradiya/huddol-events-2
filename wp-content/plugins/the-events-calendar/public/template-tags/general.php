...
if ( class_exists( 'TribeEvents' ) ) {
	 ...
	 */
	function tribe_events_event_schedule_details( $event = null, $before = '', $after = '', $lang='' ) {
		if ( is_null( $event ) ) {
			global $post;
			$event = $post;
		}
		...
		if($lang == 'en')
		{
		    ...
		    $start_time_format              = 'g:i';
		    $end_time_format = 'g:i a';
        }
        else
        {
            $microformatStartFormat   = tribe_get_start_date( $event, false, 'Y-m-dTh:i' );
		    $microformatEndFormat     = tribe_get_end_date( $event, false, 'Y-m-dTh:i' );
		    $start_time_format              = 'H\hi';
		    $end_time_format = 'H\hi';
		}
		...
		*/
		...
		$schedule .= tribe_get_start_date( $event, false, $format ) . ( $time ? $datetime_separator . tribe_get_start_date( $event, false, $start_time_format ) : '' );
		...
		$schedule .= ( $show_end_time ? tribe_get_end_date( $event, false, $end_time_format ) : '' ) . '<span class="value-title" title="' . $microformatEndFormat . '"></span>';
