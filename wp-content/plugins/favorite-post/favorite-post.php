...
class WeDevs_Favorite_Posts {
    ...
    function favorite_post() {
        ...
        if ( !$this->get_post_status( $post_id, $user_id ) ) {
            ...
            wp_send_json_success( '<span class="wpf-favorite">&nbsp;</span> ' . __( 'Remove', 'tcn' ) );
        } else {

            $this->delete_favorite( $post_id, $user_id );

            wp_send_json_success( '<span class="wpf-favorite">&nbsp;</span> ' . __( 'Add to my favorites', 'tcn' ) );
        }
    }
    ...
    function link_button( $post_id ) {
        ...
        <a class="wpf-favorite-link" href="#" data-id="<?php echo $post_id; ?>">
            <?php if ( $status ) { ?>
                <span class="wpf-favorite"><?php _e( 'Remove', 'tcn' ); ?></span> 
            <?php } else { ?>
                <span class="wpf-favorite"><?php _e( 'Add to my favorites', 'tcn' ); ?></span> 
            <?php } ?>
        </a>
    ...
    function display_favorites( $post_type = 'all', $user_id = false, $limit = 10, $show_remove = true ) {
        ...
        if ( $posts ) {
            ...
            $remove_title = __( 'Remove favorite', 'tcn' );
            ...
            printf( '<li>%s</li>', __( 'Nothing found', 'tcn' ) );
        }
        echo '</ul>';
    }
