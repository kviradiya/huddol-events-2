...
<div class="wrap">
    ...
    <td>
        <?php $mo_file = ABSPATH . (defined('WP_CONTENT_DIR')? basename( WP_CONTENT_DIR ).'/languages': LANGDIR) . '/' . $locales[$lang['code']] . '.mo'; ?>
        <?php if(@is_readable($mo_file)): ?>
        <span class="icl_valid_text"><?php echo __('File exists.', 'sitepress') ?></span>                
		<?php elseif($lang['code'] != 'en' ): ?>
        <span class="icl_error_text"><?php echo $mo_file; ?><?php echo __('File not found!', 'sitepress') ?></span>
        <?php endif; ?>
    </td>
    <?php if($sitepress_settings['theme_localization_type']==2):?>       
    <td>
        <?php echo TEMPLATEPATH . '/' . $locales[$lang['code']] . '.mo'; ?>
        <?php 
