    ...
    if($wp_version >= '3.0') {
        ...
        if (is_array($wptm_taxonomies) )
        {
            foreach ($wptm_taxonomies as $wptm_taxonomy ) {
                ...
                // add_action($wptm_taxonomy . '_add_form_fields', 'wptm_add_category_tags');
                add_action($wptm_taxonomy . '_edit_form', 'wptm_add_category_tags');
            }
        }
    } else {
...
function wptm_save_meta_tags($id) {
    ...
    if (isset($wptm_edit) && !empty($wptm_edit)) 
    {
        ...
        foreach($tags as $tag)
        {
            if(isset($_POST['tcn_category_tag_' . $tag->term_id]))
            {
                array_push($selected_tags, $tag->term_id);
            }
        }
        
        update_terms_meta($id, 'tcn_category_tags', $selected_tags);
    }	
}
...
function get_category_tagged_with($category_id, $term_id)
{
...
function wptm_add_category_tags($tag)
{   
    global $category;
    $category_id = $tag->term_id;
    ?>
    
    <table class="fixed" style="margin-top: 30px;">
        <thead>
            <tr class="title">
                <th scope="col" class="manage-column">TCN Higlighted Tags</th>
        ...
        <?php
            ...
            { 
                if(get_category_tagged_with($category_id, $tag->term_id))
                {
                    echo '<li style="float: left; width: 200px;"><label><input checked="checked" type="checkbox" name="tcn_category_tag_'. $tag->term_id . '" /><span>' . $tag->name . '</span></label></li>';
                }
                else
                {
                    echo '<li style="float: left; width: 200px;"><label><input type="checkbox" name="tcn_category_tag_'. $tag->term_id . '" /><span>' . $tag->name . '</span></label></li>';
                }
            } 
        ?>
                </ul>
