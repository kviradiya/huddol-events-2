...
class G1_Mailchimp_Module extends G1_Module {
    ...
    public function add_to_mailing_list ($ajax_data=0) {
        $return_non_ajax = false;
        if($ajax_data==0)
        {
        ...
        if(empty($api_key))
        {
            $response = __( 'Empty API Key', 'g1_theme' );
        }
        
        if(empty($mailing_list))
        {
            $response = __( 'Empty Mailing Lists', 'g1_theme' );
        }
        
        if(! is_email($subscriber_email))
        {
            $response = __( 'This is not a valid email address', 'g1_theme' );
        }
    ...
    public function enqueue_scripts() {
        ...
        $config = array(
            ...
            'i18n' => array(
                ...
                'subscription_success'          => __( 'Your email was added to our newsletter', 'g1_theme' ),
