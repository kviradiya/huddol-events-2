...
case 'resetpass' :
case 'rp' :
    $url = site_url() . "/password-reset/?key=" . $_GET['key'];
    header("location:".$url);
    break;
*/
  
// HUGE HACK HERE FROM JHILL: just redirect towards our anemic screen
...
case 'rp' :
	...
	if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
		...
		{
		    login_header( __( 'Password Reset' ), '<p class="message reset-pass">' . __( 'Your password has been reset.' ) . ' <a href="' . site_url() . '/login/' . '">' . __( 'Log in' ) . '</a></p>' );
		}
		else
		{
